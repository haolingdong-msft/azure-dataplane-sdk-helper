...
export class DotNetConversationHandler implements TeamsFxBotCommandHandler {
  triggerPatterns: TriggerPatterns = "^.net: .*";
  chain;
  ...
  async handleCommandReceived(
    ...
    console.log(`[DotNetConversationHandler]App received message: ${message.text}`);
    const question = message.text.substring(this.triggerPatterns.toString().length - " .*".length);
    console.log(`[DotNetConversationHandler]extracted question: "${question}"`);
